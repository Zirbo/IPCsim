...
void verlet(unsigned long &t, parametri & par, vec x[], vec v[], vec F[], FU_table tab, cell_lists cells)
{
  ...
  for ( int i=0; i<par.nIPCs; i++ )
  {
    vec eFp1 = F[i+par.nIPCs]*par.cP11 + F[i+par.nPatc]*par.cP12 + F[i]*par.cP1c;
    vec eFp2 = F[i+par.nIPCs]*par.cP21 + F[i+par.nPatc]*par.cP22 + F[i]*par.cP2c;
  ...
  for ( int i=0; i<par.nIPCs; i++ )
  {
    vec eFp1 = F[i+par.nIPCs]*par.cP11 + F[i+par.nPatc]*par.cP12 + F[i]*par.cP1c;
    vec eFp2 = F[i+par.nIPCs]*par.cP21 + F[i+par.nPatc]*par.cP22 + F[i]*par.cP2c;
    vec q1   = v[i+par.nIPCs] + eFp1*(.5*par.dt*par.im1);
    vec q2   = v[i+par.nPatc] + eFp2*(.5*par.dt*par.im2);
