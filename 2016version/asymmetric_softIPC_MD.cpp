...
void warmup(parametri & par, vec *& x, vec *& v, vec *& F, char *& farben, FU_table & tab, ostream & OUT, cell_lists & cells, bool restoreprevious)
{
  ...
  {
    cerr<<par.ecc1<<"+"<<par.s1Radius<<"="<<par.ecc1+par.s1Radius<<"-";
    cerr<<par.ecc2<<"+"<<par.s2Radius<<"="<<par.ecc2+par.s2Radius<<"=\n";
    cerr<<(par.ecc1+par.s1Radius)-(par.ecc2+par.s2Radius)<<endl;
    cerr<<"eccentricities and radii are not consistent!\n"; exit(1);
  }
  ...
  double Imass = 1.+(par.ecc1*par.ecc1 + par.ecc2*par.ecc2)/par.PatchDistanceSquared;
  par.cP11 = 1.-par.ecc2*par.ecc2/(par.PatchDistanceSquared*Imass);
  par.cP12 = -par.ecc1*par.ecc2/(par.PatchDistanceSquared*Imass);
  par.cP1c = par.ecc2/(par.PatchDistance*Imass);
  par.cP21 = 1.-par.ecc1*par.ecc1/(par.PatchDistanceSquared*Imass);
  par.cP22 = par.cP12;
  par.cP2c = par.ecc1/(par.PatchDistance*Imass);
  par.alpha_1 = 1. + par.ecc2*(par.ecc1-par.ecc2)/(Imass*par.PatchDistanceSquared);
  par.alpha_2 = 1. - par.ecc1*(par.ecc1-par.ecc2)/(Imass*par.PatchDistanceSquared);
          ...
          for (int i=0;i<3;i++)
          {
            for (int j=0;j<3;j++)
            {
              ...
              if (r <= par.PotRangeSquared)
              {
                ...
                else  { cerr<<"CRAAAAAAAAAAAAAAAAAAP!"<<endl; exit(1);}
...
void verlet(unsigned long &t, parametri & par, vec x[], vec v[], vec F[], FU_table tab, cell_lists cells)
{
  ...
  for ( int i=0; i<par.nIPCs; i++ )
  {
    vec eFp1 = F[i+par.nIPCs]*par.cP11 + F[i]*par.cP1c + F[i+par.nPatc]*par.cP12;
    vec eFp2 = F[i+par.nPatc]*par.cP21 + F[i]*par.cP2c + F[i+par.nIPCs]*par.cP22;
